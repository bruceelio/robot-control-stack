...
class SR2026SimSemanticMotor:
    """Canonical motor wrapper around one simulated SR Motor Board channel."""
...
class SR2026SimSemanticDrive:
    """
    Canonical paired drive interface.

    The SR2026 Webots robot exposes the two Motor Board channels separately,
    so this wrapper applies the requested left/right values as one semantic
    operation.
    """

    def __init__(
        self,
        left_motor: SR2026SimSemanticMotor,
        right_motor: SR2026SimSemanticMotor,
    ):
...
class SR2026SimPiezo:
    """
    Simulated SR Power Board piezo wrapper.
    ...
    def buzz(self, tone, duration: float, *, blocking: bool = False) -> None:
        if self._power is None:
            print(
                f"[SR2026SIM PIEZO] tone={tone} duration={duration} "
                "(no power board)"
            )
            return
...
class SR2026SimOutputs:
    """
    Compatibility digital-output interface.

    VACUUM is currently mapped to simulated SR Power Board
    high-current output H0.

    NOTE:
        This mapping belongs specifically to the supplied SR2026
        Webots robot. It must not be assumed to describe physical
    ...
    def set(self, name: str, on: bool) -> None:
        ...
        if self._power is None:
            print(f"[SR2026SimOutputs] {name} -> {on} (no power board)")
            return
...
class SR2026SimVacuumGripper:
    """
    Compatibility wrapper which presents the Webots vacuum pickup as the
    canonical gripper position device.

    Existing robot semantics:
        negative position -> GRAB / closed
        positive position -> RELEASE / open

    Webots implementation:
        GRAB    -> Power Board H0 vacuum ON
        RELEASE -> Power Board H0 vacuum OFF
    """

    def __init__(self, outputs: SR2026SimOutputs):
    ...
    def position(self, value):
        ...
        # Existing gripper convention:
        # negative = grab/closed
        # positive = release/open
        self._outputs.set("VACUUM", value < 0.0)

class SR2026SimIO(IOMap):
    """
    Student Robotics 2026 Webots simulation IO backend.

    Public interface:
        canonical io.* semantic devices

    Hardware implementation:
        SR robot3 API exposed by the SR2026 Webots simulator

    This backend is used by:
        - webots_robot only

    Physical Student Robotics hardware belongs in:
        - hw_sr2026.py

    All simulator-specific SR board addressing and fixed channel mappings
    belong here.
    """
    ...
    # --------------------------------------------------
    # SR2026 Webots board channel mapping
    # --------------------------------------------------
    ...
    # IMPORTANT:
    # These servo mappings are intentionally preserved from the original
    # hw_sr2026 backend for the initial backend split.
    #
    # They will be corrected separately once the sr2026sim backend has
    # been proven to resolve and run without changing behaviour.

    SERVO_LIFT_CHANNEL = 0
    ...
    def __init__(self, robot):
        ...
        self._outputs = SR2026SimOutputs(self._power)
        ...
        self._piezo: SR2026SimPiezo | None = None
        self._kch_compat: SR2026SimKCH | None = None
    ...
    def _init_sensors(self) -> None:
        ...
        # Webots/SR2026 simulation currently declares no ultrasonic devices.
        self._ultrasonic = ReadOnlyCollection({})
        ...
        # No SR2026 simulation encoder capability is currently configured.
        self._encoder = NamedIndexedCollection(
            ordered_items=[],
            named_items={},
        )
    ...
    def _init_actuators(self) -> None:
        ...
        if raw_motors is not None:
            ...
            front_left = SR2026SimSemanticMotor(
                raw_motors[self.MOTOR_FRONT_LEFT_CHANNEL],
                polarity=left_polarity,
            )

            front_right = SR2026SimSemanticMotor(
            ...
            front_drive = SR2026SimSemanticDrive(
                front_left,
                front_right,
            )
        ...
        if raw_servos is not None:
            # The supplied SR2026 Webots robot uses ServoBoard channel 0
            # for the lifting mechanism.
            lift = raw_servos[self.SERVO_LIFT_CHANNEL]

            # The simulated pickup mechanism is not a servo gripper.
            # It is a vacuum controlled by PowerBoard H0.
            gripper = SR2026SimVacuumGripper(self._outputs)

            self._servo = NamedIndexedCollection(
                # Preserve the existing Level2 semantic ordering:
                #   servos[0] -> lift
                #   servos[1] -> gripper
                ordered_items=[
                    lift,
    ...
    def _init_leds_and_audio(self) -> None:
        if self._kch_raw is not None:
            try:
                ...
                self._kch_compat = SR2026SimKCH(
                    self._kch_raw
                )

            except Exception as exc:
                print(
                    "[SR2026SIM LED] KCH semantic mapping unavailable: "
                    f"{exc}"
                )

        if self._power is not None:
            self._piezo = SR2026SimPiezo(self._power)
