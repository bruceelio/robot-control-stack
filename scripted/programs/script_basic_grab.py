...
class ScriptBasicGrab(Behavior):
    """
    Scripted program:
      - success advances to the next step
      - any failure returns FAILED (caller exits to autonomous)
    """
    ...
    def update(self, *, motion_backend=None, lvl2=None, **_):
        ...
        # Create primitive for this step
        if self.active is None:
            if self.step == "DRIVE":
                self.active = Drive(distance_mm=300.0)
            ...
            print(f"[SCRIPT_BASIC_GRAB] step={self.step}")

        # Update primitive with correct dependency
