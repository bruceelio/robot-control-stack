...
from io_sr_robot import sr_robot_io, SR_ROBOT_BOARD_NAME

# Start with the SR Robot board
canonical_to_pin = {}
board_mapping = {SR_ROBOT_BOARD_NAME: sr_robot_io}

# Attempt to load extra boards
try:
    from io_extra_board import extra_board_io, EXTRA_BOARD_NAME
    board_mapping[EXTRA_BOARD_NAME] = extra_board_io
except ImportError:
    print("No extra board found. Continuing with SR Robot only.")

# Merge all board mappings
    ...
    for canonical_name, pin in mapping.items():
        ...
        if canonical_name in canonical_to_pin:
            print(f"Warning: {canonical_name} already mapped "
                  f"({canonical_to_pin[canonical_name]}) – "
                  f"overriding with {pin} from {board_name}")
